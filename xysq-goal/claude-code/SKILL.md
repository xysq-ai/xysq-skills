...
## Scope
Personal vault by default. If the goal names a team, pass that team's `team_id`
to the vault tools. vault_find is single-vault per call (it does not fan out
across personal+teams), so for a cross-vault goal, run the find branch once
per vault and merge.
