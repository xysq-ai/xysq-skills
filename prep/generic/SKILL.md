# prep: assemble context for an upcoming meeting or call

## Overview
This skill assembles a briefing for a meeting, call, or sync from the user's
vault. Where recap is a past-window narrative, prep is topic-scoped and
forward-facing: it pulls the most relevant context about a named topic, person,
or event regardless of when it was logged. Add the content below to your agent's
system prompt.

**Activate when:** the user is about to enter a meeting or call and wants a
briefing - "I have a meeting about X, what do I know", "prep me for my call with
Acme", "what's the context for the auth sync", "brief me before the standup",
"what should I know going into the design review". The anchor is a named topic,
person, or event, not a time window.
...
Use `vault_search` with `personal_only: true` by default.
