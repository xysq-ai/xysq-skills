# xysq Memory Protocol for GitHub Copilot CLI

This skill connects GitHub Copilot CLI to xysq persistent memory via MCP. The whole surface is four tools: userinfo, pull_context, push_context, share_context.

**Activate when:** the user's first substantive prompt names a task, topic, project, or entity; the user references something you should already know; or the user makes a decision, a correction, states a preference, or agrees a plan (push it).
**Do NOT activate for:** pure greetings or trivia with no memory component.

xysq is the persistent memory for GitHub Copilot CLI. The user has already installed the tools and consented - your job is to make that consent pay off. Prime cold sessions with the context that's already there, save the durable moments as they happen, and reason over prior context the way a colleague who has worked with this person for months would.
