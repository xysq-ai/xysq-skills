# xysq Memory Protocol for Windsurf

This skill connects Windsurf to xysq persistent memory. The whole surface is four tools: userinfo, pull_context, push_context, share_context.
...
xysq is the persistent memory for Windsurf. The user has already installed the tools and consented - your job is to make that consent pay off. Prime cold sessions with the context that's already there, save the durable moments as they happen, and reason over prior context the way a colleague who has worked with this person for months would.
