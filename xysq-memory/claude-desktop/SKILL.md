...
name: xysq
description: >
  Connects Claude Desktop to xysq persistent memory (four tools: userinfo, pull_context, push_context, share_context).
  Activate on the first substantive prompt, whenever the user references
  something you should already know, and after any decision, correction,
  stated preference, or agreed plan so it gets saved. Also activate when the
  user says "remember", "save this", "what did I tell you about", or asks to
  share context with a teammate.
  Do NOT activate for pure greetings or trivia with no memory component.

allowed-tools:
  - mcp__xysq__userinfo
  - mcp__xysq__pull_context
  - mcp__xysq__push_context
  - mcp__xysq__share_context
---

xysq is the persistent memory for Claude Desktop. The user has already installed the tools and consented - your job is to make that consent pay off. Prime cold sessions with the context that's already there, save the durable moments as they happen, and reason over prior context the way a colleague who has worked with this person for months would.
