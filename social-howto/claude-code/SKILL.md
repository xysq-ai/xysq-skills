...
`mode` is one of `"foryou"`, `"following"`, or `"trending"`. You can re-call
with a `cursor` to page forward. Stop paging when either the cursor is null or
the user says they've seen enough.
